...
// export async function saveImageToStorage(file) {
//   const name = await diskStorage({
//     destination: './static/uploads/images',
//     filename: (req, file, cb) => {
//       console.log('файл есть -', file);
...
export const isFileExtensionSafe = (
  ...
  return from(FileType.fileFrom(fullFilePath)).pipe(
    switchMap(
      (fileExtensionAndMimeType: {
        ...
        const isFileTypeLegit: boolean = validFileExtensions.includes(
          fileExtensionAndMimeType.ext,
        );
        const isMimeTypeLegit: boolean = validMimeTypes.includes(
