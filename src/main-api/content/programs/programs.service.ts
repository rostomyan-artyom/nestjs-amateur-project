...
export class ProgramsService {
  constructor(
    @InjectModel(Program) private programsRepository: typeof Program,
  ...
  async getById(id: number): Promise<Program> {
    return await this.programsRepository.findByPk(id, {
      include: { all: true },
    });
  }
  ...
  async create(program: CreateProgramDto): Promise<Program> {
    const candidate: Program = await this.programsRepository.findOne({
      where: {
        alias: program.alias,
      },
    });
    ...
    const errors: any[] = await this.validateCandidate(program, candidates);

    if (errors && errors.length) {
      throw new HttpException(JSON.stringify(errors), HttpStatus.BAD_REQUEST);
    }

    return this.programsRepository.create(program);
  }

  async editById(id: number, data: CreateProgramDto): Promise<Program> {
    const candidates = [
      {
        service: this.imagesService,
        key: 'imageId',
